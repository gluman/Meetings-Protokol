...

{
  ...
  "questions": [
    {"q_number": 1, "q_title": "...", "q_summary": "краткая констатация факта"},
    ...
  ],
  "decisions": [
...
5. Вопросы: раздел «Обсудили на встрече» — КРАТКО, БЕЗ ДЕТАЛЕЙ, только констатация фактов.
   q_title — название пункта (1-3 слова), q_summary — 1-2 предложения максимум,
   без пересказа деталей, без «было отмечено что», «обсуждена необходимость» и т.п.
   Примеры: «Выбран поставщик», «Согласован бюджет», «Утверждён план-график».
6. Решения: формулировки, по которым есть явное согласие. d_text в безличной форме.
   d_owner — ФИО ответственного (ОБЯЗАТЕЛЬНО), d_due — срок в формате ДД.ММ.ГГГГ.
   Если в транскрипте звучала конкретная дата — ставь её. Если срок назван словами
   («до конца недели», «на следующей неделе») — переводи в ДД.ММ.ГГГГ от даты встречи.
   Если срок вообще не упоминался — выводи разумный дефолт «10 рабочих дней» от даты встречи.
   Никогда не оставляй d_due пустым в принятых решениях.
7. Открытые вопросы: что НЕ было решено. Формат как у решений. d_due может быть пустым.
